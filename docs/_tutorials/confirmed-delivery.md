...
This tutorial builds on the basic concepts introduced in [Persistence with Queues]({{ site.baseurl }}/persistence-with-queues) tutorial and will show you how to properly process publisher acknowledgements. Once an acknowledgement for a message has been received and processed, you have confirmed your persistent messages have been properly accepted by Solace messaging.

## Understanding Acknowledgements

An acknowledgement from Solace messaging simply means that the message has been accepted. If there are no subscriptions to the destination that message is published to, the message, by default, will be discarded. This default behavior fully decouples producers from consumers. Refer to the [Solace documentation](https://docs.solace.com/Configuring-and-Managing-Routers/Configuring-Client-Profiles.htm#Configur) for details on how to change this behavior.
